...
app.post("/api/auth/register", (req, res) => {
  ...
  try {
    ...
    res.json({ token: sign(user as any), user });
  } catch {
...
app.get("/api/orders/:id", auth, (req, res) => {
  ...
  const u = req.user!;
  if (u.role === "customer" && o.customer_id !== u.id) return res.status(403).json({ error: "غير مصرح" });
  if (u.role === "courier") {
...
app.post("/api/orders", auth, role("customer"), (req, res) => {
  const { pickup_address, delivery_address, distance_km = 2, offered_price } = req.body;
  ...
  db.prepare(
    "INSERT INTO orders(id,customer_id,pickup_address,delivery_address,distance_km,offered_price,final_price,status,confirmation_code,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?)"
  ).run(id, req.user!.id, pickup_address, delivery_address, d, offered_price ?? null, price, "pending", code, now, now);
  res.json({ ok: true, id, final_price: price, confirmation_code: code });
});
...
app.post("/api/orders/:id/accept", auth, role("courier"), (req, res) => {
  ...
  r.changes ? res.json({ ok: true }) : res.status(409).json({ error: "الطلب غير متاح" });
});
