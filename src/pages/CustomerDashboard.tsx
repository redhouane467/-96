...
function NewOrderForm({
  ...
  async function submit(
    ...
    if (!pickup || !delivery) {
      toast(
        "حدّد نقطتي الاستلام والتسليم على الخريطة",
        "error"
      );
      return;
    }
    ...
    >
      <div className="bg-white rounded-2xl p-4 shadow-sm space-y-3">
        <div className="flex items-center justify-between">
          <p className="text-sm font-bold text-slate-700">
            {pickingMode === "pickup"
              ? "اضغط على الخريطة لتحديد نقطة الاستلام"
              : "اضغط على الخريطة لتحديد نقطة التسليم"}
          </p>
      ...
      <div className="bg-white rounded-2xl p-5 shadow-sm space-y-3">
        ...
        <p className="text-xs text-slate-500">
          150 دج حتى 2 كم، ثم +50 دج لكل كم إضافي
        </p>
        ...
        <button
          disabled={submitting}
          className="w-full bg-green-600 text-white rounded-xl p-3 font-bold disabled:opacity-60"
        >
          {submitting
            ? "..."
            : "تأكيد الطلب"}
    ...
    <div className="space-y-4">
      ...
      {complaints.length === 0 ? (
        ...
        complaints.map((c) => (
          <div
            key={c.id}
            className="bg-white rounded-2xl p-4 shadow-sm space-y-1"
          >
            <p>{c.message}</p>
