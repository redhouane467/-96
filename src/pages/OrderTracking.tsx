...
export default function OrderTracking({ orderId, onClose }: { orderId: string; onClose: () => void }) {
  ...
  if (error || !order)
    return (
      <div className="space-y-3">
        <div className="bg-red-50 text-red-700 rounded-xl p-3 text-sm">{error || "الطلب غير موجود"}</div>
  ...
  if (order.pickup_lat != null && order.pickup_lng != null)
    markers.push({ id: "pickup", lat: order.pickup_lat, lng: order.pickup_lng, color: "#16a34a", emoji: "📦", popupText: "نقطة الاستلام" });
  if (order.delivery_lat != null && order.delivery_lng != null)
    markers.push({ id: "delivery", lat: order.delivery_lat, lng: order.delivery_lng, color: "#dc2626", emoji: "🏁", popupText: "نقطة التسليم" });

  const courierLocationStale =
    order.courier?.location_updated_at && Date.now() - new Date(order.courier.location_updated_at).getTime() > LOCATION_STALE_MS;

  if (order.courier?.lat != null && order.courier?.lng != null && !courierLocationStale) {
    markers.push({ id: "courier", lat: order.courier.lat, lng: order.courier.lng, color: "#2563eb", emoji: "🛵", popupText: order.courier.name });
  }
  ...
  />
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 text-center">
          ...
          {nearbyCount != null && (
            <div className="text-sm text-amber-700 mt-1">
              {nearbyCount > 0 ? `${nearbyCount} مندوب متاح قريب من موقعك` : "لا يوجد مندوبون متاحون قريبًا حاليًا"}
            </div>
          )}
        </div>
      )}

      {order.courier && (order.status === "accepted" || order.status === "picked_up" || order.status === "delivered") && (
        <div className="bg-white rounded-2xl p-4 shadow-sm flex items-center justify-between">
          ...
          <div className="text-left">
            {courierLocationStale ? (
              <span className="text-xs bg-slate-100 text-slate-500 rounded-full px-2 py-1">الموقع غير محدّث حاليًا</span>
      ...
      {order.status === "accepted" && order.confirmation_code && (
        <div className="bg-amber-50 text-amber-800 rounded-xl p-3 text-sm text-center">
          أعطِ المندوب رمز التأكيد عند الاستلام: <b className="text-lg">{order.confirmation_code}</b>
        </div>
      )}

      <div className="bg-white rounded-2xl p-4 shadow-sm space-y-1 text-sm">
        {order.package_description && <div>📦 {order.package_description}</div>}
        {order.recipient_phone && (
          <div dir="ltr" className="text-right">
            📞 {order.recipient_phone}
          </div>
        )}
        {order.notes && <div className="text-slate-500">📝 {order.notes}</div>}
